...
### read image
filepath = ['5.jpg']
target = cv2.imread(filepath[0])
target = cv2.cvtColor(target, cv2.COLOR_BGR2GRAY)
if target.shape[0]>1000 or target.shape[1]>1000:
    target = cv2.resize(target,(1000,1000))

### preprocessing
thresh = cv2.adaptiveThreshold(target,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
            cv2.THRESH_BINARY,5,15)

cv2.imshow('threshed',thresh)
cv2.waitKey(0)
cv2.destroyAllWindows()

thresh = cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)

cv2.imshow('target',thresh[0])
